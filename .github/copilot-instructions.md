...
## Dev workflow (local)
- Prereqs: MongoDB (mongodb://localhost:27017) and MinIO (127.0.0.1:9000). Backend env in packages/backend/.env (see backend README): MINIO_*, MEDIA_PRESIGNED_MODE, etc.
...
- Storage (src/services/storage_minio.py): Keys "{library_id}/{image_id}.ext"; thumbs are JPEG "...jpg". Use presign_* for redirect/url modes; delete_by_prefix for cleanup.
